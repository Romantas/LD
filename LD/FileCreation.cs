...
namespace LD
{
    class FileCreation
    {
        ...
        public void separationTwo()
        {
            stopWatch.Restart();
            Console.WriteLine("List TEST two");
            List<Student> studentsList = new List<Student>(studentsArray);
            List<Student> studentsFailed = new List<Student>();
            ...
            studentsList.RemoveAll(s => s.final_points < 5);

            TimeSpan ts = stopWatch.Elapsed;
            timeSpend(ts, "iskaidymas LIST");

            stopWatch.Restart();
            Console.WriteLine("Linked TEST TWO");
            LinkedList<Student> studentsFailedLinked = new LinkedList<Student>();

            var studentsLinked = new LinkedList<Student>(studentsArray);
            var node = studentsLinked.First;
            while (node != null)
            {
                var next = node.Next;
                if (node.Value.final_points < 5)
                    studentsLinked.Remove(node);
                node = next;
            }
            ...
            stopWatch.Restart();
            Console.WriteLine("Que TEST TWO");
            Queue<Student> studentsQue = new Queue<Student>(studentsArray);
            Queue<Student> studentsFailedQue = new Queue<Student>();
            Queue<Student> studentsPassedQue = new Queue<Student>();
            ...
            studentsQue = new Queue<Student>(studentsQue.Where(s => s.final_points >= 5));
            ts = stopWatch.Elapsed;
            timeSpend(ts, "iskaidymas Que TWO");
        }
